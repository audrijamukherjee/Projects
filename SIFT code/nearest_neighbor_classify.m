...
Distances = vl_alldist2( test_image_feats.',train_image_feats.');
%Distances(i,j)= Distance between test i and train j image
%for each test image get nearest neighbour
[match, I_match]=sort(Distances,2);
%for 1NN, first column of I_match is the predicted category
% UNCOMMENT FOR 1NN 
%predicted_categories=train_labels(I_match(:,1));
k=1; %taking k nearest neighbours

%assuming M=N
predicted_categories = cell(size(test_image_feats,1),1);
nearest_neighbours=train_labels(I_match(:,1:k));
for i=1:size(test_image_feats,1)
    [unique_strings, ~, string_map]=unique(nearest_neighbours(i,:));
    %s=unique_strings(mode(string_map));
