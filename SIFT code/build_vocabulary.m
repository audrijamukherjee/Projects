...
for i=1:N
    ...
    [locations, SIFT_ind] = vl_dsift(single(image),'STEP',5,'Fast');
    if SIFT_all==0
        SIFT_all=SIFT_ind; %SIFT_features is a 128 x N matrix
    else
        SIFT_all=cat(2,SIFT_all, SIFT_ind);%each column is a feature
    end
end
%SIFT_all has all extracted features, one column is one feature
% The output 'vocab' should be vocab_size x 128. Each row is a cluster
% centroid / visual word.
[vocab, assignments] = vl_kmeans(single(SIFT_all), vocab_size);
fprintf('After K means');
vocab=vocab.';
